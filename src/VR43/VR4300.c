...
const uint32_t CART_ADDRS[5] =   {0x10000000, 0x90000000, 0xB0000000, 0xD0000000, 0xF0000000};
const uint32_t RDRAM_ADDRS[5] =  {0x00000000, 0x80000000, 0xA0000000, 0xC0000000, 0xE0000000};
const uint32_t RSP_DMEM_ADDRS[5] = {0x04000000, 0x84000000, 0xA4000000, 0xC4000000, 0xE4000000};
...
void map_rom(VM_state **state, char *rom_name){
...
void map_rsp_dmem(VM_state **state){
    int rsp_dmem_fd = memfd_create("rdram", 0);
    if (rsp_dmem_fd == -1) {
      fprintf(stderr, "Could not set create rsp dmem file descriptor\n");
      exit(errno);
    }
    if (ftruncate(rsp_dmem_fd, RSP_DMEM_LEN) == -1){
      fprintf(stderr, "Could not set size of rsp dmem file descriptor\n");
      exit(errno);
    }
    for(int i = 0;i<5 ;i++){
...
void build_vm_state(VM_state **state, char *rom_name) {
  *state = (VM_state *)calloc(1, sizeof(VM_state));
  ...
  (*state)->gprs[20] = 0x00000001;
  (*state)->gprs[22] = 0x0000003F;
  (*state)->gprs[29] = 0xA4001FF0;
...
void HALT(VM_state *vm) {
  ...
  uint8_t instr = rearrange(*(uint8_t *)vm->pc);
  printf("rearranged instr %08bb %d \n", instr, instr);
  printf("HALTED at addr  %08llX\n", (vm->pc));
  for(int i = 0; i<5; i++){
      munmap(vm->rsp_dmem_mem[i], RSP_DMEM_LEN);
      munmap(vm->cart_mem[i], vm->cart_size);
      munmap(vm->rdram[i], RDRAM_SIZE);
...
static inline __attribute__((always_inline)) void fetch_decode(VM_state *vm) {
  ...
  //        rearrange(*pc));
  *(vm->gprs) = 0;
  uint32_t addr = (uint32_t)(((uintptr_t)(vm->pc)));
  //uint32_t virtual_addr = (uint32_t)(((uintptr_t)(vm->pc)));
  //uint32_t physical_addr = addr_translation(virtual_addr);
  fprintf(stdout, "addr:%08llX\n",addr);
...
void begin(VM_state *state, SDL_Window *window) {
  ...
  state->pc = (uint32_t *)(INITIAL_PC + HEADER_SIZE);
